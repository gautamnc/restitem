...
public abstract class AbstractBase {
	...
	public void setUp() throws Exception {
		...
		String restServer = System.getenv("restServer");
		if (null== restServer || restServer.equals("")){
			restServer = "localhost";
		}
		
		RestAssured.baseURI = "http://"+ restServer;

		String overridePort = System.getenv("overridePort");
		if (null != overridePort || "".equals(overridePort)) {
			RestAssured.port = Integer.parseInt(overridePort);
		} else {
			RestAssured.port = 7070;
		}

		if (RestAssured.baseURI.contains("localhost")) {
			RestAssured.basePath = "/restitem/services/"; // because mostly
															// local deploys are
															// in app specific
															// folders
		} else {
		...
		given().expect().statusCode(200).log().ifError().when()
				.get("/system/status");
	}
}
